...
// Fully-unrolled architecture: Instantiates all 24 rounds in hardware combinatorially.
// Highest throughput but highest resource usage. Processes blocks through SYCL pipes.
...
// Define SYCL pipes for streaming data
...
// Fully-unrolled Keccak-f[1600]: All 24 rounds instantiated in hardware
void keccak_f1600_fully_unrolled(std::array<uint64_t, 25>& state) {
    uint64_t C[5], D[5];
    uint64_t t0, t1, t2, t3, t4;

    // All 24 rounds in silicon
    #pragma unroll 24
    for (int i = 0; i < 24; ++i) {
...
int main() {
    try {
        ...
        std::cout << "Synthesising Fully-Unrolled Keccak-f[1600] hardware pipeline with Pipes...\n";
        ...
        q.submit([&](handler& h) {
            h.single_task<class KeccakPipeline>([=]() [[intel::kernel_args_restrict]] {
                for (int i = 0; i < num_blocks; ++i) {
                    ...
                    keccak_f1600_fully_unrolled(local_state);
